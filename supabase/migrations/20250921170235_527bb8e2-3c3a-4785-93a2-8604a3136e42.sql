...
-- Add unique constraints to prevent duplicates where applicable
...
-- Create duplicate detection function for admins
CREATE OR REPLACE FUNCTION public.detect_duplicates(table_name TEXT DEFAULT NULL)
RETURNS TABLE(
  table_name TEXT,
...
BEGIN
  ...
  IF table_name IS NULL OR table_name = 'customers' THEN
    RETURN QUERY
    SELECT 
      'customers'::TEXT,
      'email'::TEXT,
  ...
  IF table_name IS NULL OR table_name = 'sales_invoices' THEN
    RETURN QUERY
    SELECT 
      'sales_invoices'::TEXT,
      'invoice_number'::TEXT,
  ...
  IF table_name IS NULL OR table_name = 'users' THEN
    RETURN QUERY
    SELECT 
      'users'::TEXT,
      'email'::TEXT,
...
BEGIN
  ...
  IF table_name = 'sales_invoices' THEN
    WITH duplicates AS (
      SELECT id, ROW_NUMBER() OVER (

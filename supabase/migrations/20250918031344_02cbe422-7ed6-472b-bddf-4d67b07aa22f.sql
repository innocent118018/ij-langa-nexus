-- Create WhatsApp templates table for message templates
...
CREATE POLICY "Super admins can view all activity logs" ON public.admin_activity_logs
FOR SELECT TO authenticated
USING (EXISTS (
  SELECT 1 FROM public.users 
  WHERE users.id = auth.uid() 
...
INSERT INTO public.whatsapp_templates (name, category, template_body, header_content, footer_text, button_type, button_text) VALUES
('login_otp_notification', 'Authentication', 
'Hello {{1}},
...
-- Create trigger for updated_at on whatsapp_templates
